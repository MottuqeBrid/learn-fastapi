...
class UserInput(BaseModel):
    age: Annotated[
        int, Field(..., gt=0, lt=120, description="Age of the patient", examples=[30])
    ]
    weight: Annotated[
        float,
    ...
    income_lpa: Annotated[
        float,
        Field(..., gt=0, description="Income level per annum", examples=[50000.0]),
    ]
    smoker: Annotated[
        bool, Field(..., description="Whether the user smokes", examples=[False])
    ]
    city: Annotated[
        str, Field(..., description="City of residence", examples=["Nagpur"])
    ]
